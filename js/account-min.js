...
function checkAdmin(){
    var data = nFetch('https://api.nitlix.pro/api/v1/admin?session='+localStorage.getItem('account-session')+'&action=check')
...
function adminFunction(action){
    var data = nFetch('https://api.nitlix.pro/api/v1/admin?session='+localStorage.getItem('account-session')+'&action='+action);
...
window.onload=function(){
    ...
    setInterval(()=>{
        ...
        if (adminlogin){
            ...
            argstr = "&"+arglist.join("")
            var data = nFetch('https://api.nitlix.pro/api/v1/admin?action=status&session='+localStorage.getItem('account-session')+argstr)
